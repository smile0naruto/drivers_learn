...
//#define Enable_KKDRV_LOG 3
#define KK_TEST_CRTI 1
#define KK_TEST_FULL 2

static unsigned int kk_log_state_flag = 3;

#define kk_log(num, fmt, args...) \
do {\
	if (kk_log_state_flag >= (int)num) \
		switch (num) {\
		case KK_TEST_CRTI:\
				 pr_err(fmt, ##args); \
				 break; \
...
//		int res;
		...
		if (ret != 1) {
				kk_log(KK_TEST_CRTI,"kpd call state: Invalid values\n");
				return -EINVAL;
		}
		if (kk_test_state1 == 1){
...
//kk_log_state
#ifdef DEVICE_CREAT
static ssize_t kk_log_show_state(struct device *dev, struct device_attribute *attr, char *buf)
#else
static ssize_t kk_log_show_state(struct device_driver *driver, char *buf)
#endif
{
	ssize_t res;

	kk_log(KK_TEST_FULL,"=====%s====%d==\n\n",__func__,kk_log_state_flag);
	res = snprintf(buf, PAGE_SIZE, "%d\n", kk_log_state_flag);
	return res;
}
#ifdef DEVICE_CREAT
static ssize_t kk_log_store_state(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
#else
static ssize_t kk_log_store_state(struct device_driver *driver, const char *buf,size_t count)
#endif
{
		int ret;
//		int res;
		ret = sscanf(buf, "%d", &kk_log_state_flag);
		if (ret == -1) {
				kk_log(KK_TEST_CRTI,"kk_test state: Invalid values\n");
				return -EINVAL;
		}
		kk_log(KK_TEST_CRTI, "the kk_log_state_flag is %d \n\n",kk_log_state_flag);
...
static DEVICE_ATTR(kk_log_state,  0644, kk_log_show_state, kk_log_store_state); 
#else
static DRIVER_ATTR(kk_test_state, 0644, kk_test_show_state, kk_test_store_state);
static DRIVER_ATTR(kk_log_state,  0644, kk_log_show_state, kk_log_store_state); 
#endif

#ifndef DEVICE_CREAT
static struct driver_attribute *kk_test_attr_list[] = {
	&driver_attr_kk_test_state,
	&driver_attr_kk_log_state,        
};
...
static int kk_test_create_attr(struct device_driver *driver)
{
	...
	if (driver == NULL)
			kk_log(KK_TEST_CRTI,"there is not real driver,just sys attr???!!!\n\n");//		return -EINVAL;

	kk_log(KK_TEST_FULL,"%s,num is %d \n\n",__func__,num);
	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, kk_test_attr_list[idx]);
		if (err) {
			kk_log(KK_TEST_CRTI,"222driver_create_file (%s) = %d\n", kk_test_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
static int kk_test_get_gpio_info(struct platform_device *pdev)
{
		...
		if (IS_ERR(pinctrl_kk_test_def)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_def pinctrl!\n");
		}		
		...
		if (IS_ERR(pinctrl_kk_test_def_low)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_def_low pinctrl!\n");
		}
		...
		if (IS_ERR(pinctrl_kk_test_def_high)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_def_high pinctrl!\n");
		}
		...
		if (IS_ERR(pinctrl_kk_test_set)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_set pinctrl!\n");
		}	
		...
		if (IS_ERR(pinctrl_kk_test_set_low)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_set_low pinctrl!\n");
		}
		...
		if (IS_ERR(pinctrl_kk_test_set_high)) {
				...
				kk_log(KK_TEST_CRTI,"Cannot find pinctrl_kk_test_set_high pinctrl!\n");
		}		
...
static int kk_test_check_bl_status(const char *filename)
{
	...
	if (IS_ERR(bl)) {
			kk_log(KK_TEST_CRTI,"===kk_test open %s failed!!!\n",filename/*,strerror(errno)*/);
			return -1;
	} else {
	//	ret = bl->f_op->llseek(bl, 0, SEEK_SET);
		len = bl->f_op->read(bl,buf,1,bl->f_op);
		if (len < 0){
			kk_log(KK_TEST_CRTI,"===kk_test read %s buf failed!!\n",filename);
			return -1;
		}
		kk_log(KK_TEST_CRTI,"===kk_test check the buf[0] is %d ==len is %d =\n",buf[0],len);
		if ( buf[0] == '0')
			kk_test_bl_status = 0;
		else if ( buf[0] > '0')
			kk_test_bl_status = 1;
...
#endif
		kk_log(KK_TEST_CRTI,"====kk_test===check io is %d =%d===\n",ret,bl_status);
		...
		if ( bl_status == 1 ){
			if (ret == 1){
    			...
    			kk_test_flag = 1;     
  				...
  				kk_log(KK_TEST_CRTI,"===konka===1===\n");
  		}
  	}else if (bl_status == 0){
  		if ( ret == 1)
  				kk_log(KK_TEST_CRTI,"===konka===2===\n");
  		else if (ret == 0){
  	 			input_report_key(kk_test_input_dev, KEY_POWER, 1);
    			input_sync(kk_test_input_dev);       
    			input_report_key(kk_test_input_dev, KEY_POWER, 0);
...
{   
    ...
    { 
		...
		if (ret < 0)
				kk_log(KK_TEST_CRTI,"mt_hall_init gpio request error!\n");
		ret = gpio_direction_input(kk_test_en);
		if (ret < 0)
				kk_log(KK_TEST_CRTI,"mt_hall_init gpio direction error!\n");
		ret = gpio_to_irq(kk_test_en);
		if (ret < 0)
				kk_log(KK_TEST_CRTI,"mt_hall_init gpio to irq error!\n");
...
static int kk_test_probe(struct platform_device *pdev)
{
    ...
    if (err){
        	kk_log(KK_TEST_CRTI,"%s regist input device failed!!!\n",__func__);
    ...
    { 
		...
		if (err < 0 )
				kk_log(KK_TEST_CRTI,"Failed to create device file(%s)!\n", dev_attr_kk_test_state.attr.name);
#else 
...
{
	int ret;
	kk_log(KK_TEST_CRTI,"====enter ko world==\n");
	
  ret = platform_driver_register(&kk_test_drv);
  if (ret) {
  		kk_log(KK_TEST_CRTI,"register driver failed (%d)\n", ret);
      return ret;
  }
...
{
	kk_log(KK_TEST_CRTI,"===%s==goodbye ====\n",__func__);
	platform_driver_unregister(&kk_test_drv);
	kk_log(KK_TEST_CRTI,"===%s==exit ====\n",__func__);
}
